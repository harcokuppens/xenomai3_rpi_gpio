...
void current_set_affinity_cpu( int cpu) {
    cpu_set_t cpus;
    CPU_ZERO(&cpus);          
    CPU_SET(cpu,&cpus);          
    int ret=rt_task_set_affinity(NULL,&cpus);  
	rt_printf("ret after set affinitiy %d\n",ret);
}
...
void runBlink(void *arg) {
    rt_task_sleep(500000000); // allow runHello to run first for small time

	rt_printf("Starting blink...\n");
	rt_printf("sleep blink task first %llu nanoseconds\n",initialSleepToReadOutTaskList);
    current_set_affinity_cpu(2);  // ok, makes hello world run in parallel

    //rt_timer_spin(initialSleepToReadOutTaskList);
...
void runHello(void *args) {
    ...
    current_set_affinity_cpu(1);  // ok, makes hello world run in parallel
