...
void blink( int repeat, int timeout) {
    ...
    do {
        ...
        //GPIORead(fd_out_write,POUT);  // not allowed for fd_out_write both for RDONLY as for RDWR  mode -> reading also not really needed
        //GPIORead(fd_in,PIN); // already used for interrupt handler
...
int run() {
    ...
    printf("all realtime tasks initially start with sleep\nso that you have the time to lookup priority and cpu and irq at:\n    /proc/xenomai/sched/rt/threads\n    /proc/xenomai/irq\n\n"); 
    ...
    //fd_out_read=GPIOReadInit(POUT); // not allowed anymore to open device twice! (was allowed in xenomai 3.05)
